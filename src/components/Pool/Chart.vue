...
import { swapPrice, getLbpData } from "@/config/lbpData";
	...
	computed: {
		...
		items() {
			...
			if (this.displayPriceHistory) {
				tabList.push({
					name: this.$t("priceHistory", {
						token: this.lbpData.projectToken,
					}),
					id: "PRICE",
				});
			}
		...
		chartData() {
			...
			if (this.activeTab == "PRICE") {
				for (let i = 0; i < this.swaps.length; i++) {
					...
					if (
						i > 0 &&
						swap.timestamp != this.swaps[i - 1].timestamp
					) {
						data.push({
							time: swap.timestamp,
							value: swapPrice(
